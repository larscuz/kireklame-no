...
import { assertCompanyEditorApi } from "@/lib/supabase/server";
...
export async function POST(req: Request) {
  try {
    ...
    // ✅ API-safe auth: ingen redirect()
    let isAdmin = false;
    try {
      const auth = await assertCompanyEditorApi(companyId);
    ...
    // Filtype whitelist (matcher upload.php)
    ...
    if (!upRes.ok || !payload?.ok || !payload?.url) {
      return NextResponse.json(
        { error: "Upload failed", status: upRes.status, response: payload ?? text },
        { status: 502 }
      );
    }
    ...
    // Oppdater companies.cover_image i Supabase DB
    ...
    const next = isAdmin ? `/admin/companies/${companyId}` : `/me/company/${companyId}`;
