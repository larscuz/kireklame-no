...
export const metadata = siteMeta({
  ...
  description:
    "Internasjonale saker i KiR Nyheter: AI-first, AI-only og global utvikling i reklame, byrå og media.",
...
function StoryVisual({ article, className }: { article: NewsArticle; className: string }) {
  if (hasImage(article.hero_image_url)) {
    return <NewsImage src={article.hero_image_url ?? ""} alt={article.title} className={className} />;
  }
...
export default async function KIRInternationalPage() {
  ...
  const desk = allNews.filter((article) => isLikelyInternationalDeskArticle(article));
  const deskWithImage = desk.filter((item) => hasImage(item.hero_image_url));
  const sourcePool = deskWithImage.length > 0 ? deskWithImage : desk;

  const lead = sourcePool[0] ?? null;
  const rest = lead ? sourcePool.filter((item) => item.id !== lead.id) : sourcePool;

  const used = new Set<string>();
  const sidebarTop = takeFromPool(rest, used, 5);
  const sidebarAfterAd = takeFromPool(rest, used, 6);
  const frontRow = takeFromPool(rest, used, 2);
  const frontNow = takeFromPool(rest, used, 3);
  const latest = takeFromPool(rest, used, 7);
  const frontFillRest = takeFromPool(rest, used, 9);

  const remaining = rest.filter((item) => !used.has(item.id));
  ...
  const usedInBottom = new Set<string>([
    ...used,
    ...criticism.map((item) => item.id),
    ...adoption.map((item) => item.id),
    ...analysis.map((item) => item.id),
  ]);
  const archiveRows = rest.filter((item) => !usedInBottom.has(item.id));
      ...
      <header className="border-y border-black/20 bg-[#f6f2e9]">
        <div className="mx-auto max-w-[1260px] border-b border-black/15 px-3 py-2 text-[10px] uppercase tracking-[0.21em] text-black/65 md:px-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span>Internasjonal desk · Global dekning</span>
        ...
        <div className="mx-auto max-w-[1260px] px-3 py-4 md:px-4 md:py-5">
          <div className="grid gap-2 border-b border-black/20 pb-3 md:grid-cols-[1fr_auto_1fr] md:items-end">
            ...
            <div className="min-w-0 text-center">
              <div className="mx-auto mb-2 flex items-center justify-center gap-3">
                ...
                />
                <span className={`${masthead.className} text-[clamp(2.3rem,10.8vw,3.45rem)] leading-[0.92] sm:text-6xl md:text-7xl`}>
                  Internasjonalt
                </span>
              </div>
              <p className="text-[10px] uppercase tracking-[0.28em] text-black/60 md:text-[11px]">
                KiR Nyheter · Global AI-desk
              </p>
            </div>
          ...
          <nav className="mt-3 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/72 sm:text-[11px] md:gap-5 md:tracking-[0.2em]">
            <a href="#internasjonalt" className="hover:text-black">
              Internasjonalt
            </a>
      ...
      <main className="mx-auto max-w-[1260px] overflow-x-clip px-3 py-4 md:px-4 md:py-5">
        ...
        {lead ? (
            ...
            <div className="min-w-0 border-b border-black/15 pb-4 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-4">
              <article>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.13em] text-black/55">
                  ...
                  <span>{lead.language ? lead.language.toUpperCase() : "EN"}</span>
                  ...
                  <div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {frontNow.map((article) => (
                      <article key={article.id} className="border border-black/15 bg-[#f8f4eb] p-2">
                        ...
                        />
                        <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/58">
                          {internationalLabel(article)}
          ...
          <section className="border border-black/20 bg-[#f8f4eb] p-7 text-center">
            <h2 className={`${headline.className} text-4xl`}>Ingen internasjonale saker ennå</h2>
        ...
        {archiveRows.length ? (
          <section className="mt-5 border-t border-black/20 pt-3">
            <h2 className={`${masthead.className} text-[29px] leading-none sm:text-[34px]`}>
              Alle internasjonale saker
            </h2>
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {archiveRows.map((article) => (
                <article key={article.id} className="grid grid-cols-[72px_1fr] gap-2 border-b border-black/10 pb-2 sm:grid-cols-[84px_1fr]">
                  <StoryVisual
                    article={article}
                    className="aspect-square w-full border border-black/20 object-cover"
                  ...
                  <div className="min-w-0">
                    ...
                    <h3 className={`${headline.className} mt-1 text-[21px] leading-[1.01] [overflow-wrap:anywhere] hyphens-auto sm:text-[24px]`}>
                      <Link href={`/ki-avis/${article.slug}`} className="hover:opacity-75">
                        {article.title}
                      </Link>
                    </h3>
        ...
        <section className="mt-5 border-t border-black/20 pt-4">
          <h2 className={`${masthead.className} text-[29px] leading-none sm:text-[34px]`}>
            Fagbegreper som beholdes på engelsk
          </h2>
