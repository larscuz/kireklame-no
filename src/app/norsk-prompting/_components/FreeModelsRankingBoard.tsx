...
function ScoreChip({ item }: { item: FreeModelRankingItem }) {
  ...
  return (
    <div className="group relative">
      <span
        className="inline-flex cursor-help items-center gap-1 rounded-lg border border-zinc-300/35 bg-zinc-300/10 px-2 py-1 text-xs font-semibold text-zinc-100"
      ...
      <div className="pointer-events-none absolute right-0 top-full z-[80] mt-2 w-56 translate-y-1 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-2 opacity-0 shadow-[0_12px_28px_rgba(2,6,23,0.4)] ring-1 ring-zinc-900/30 transition group-hover:translate-y-0 group-hover:opacity-100">
        <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[rgb(var(--muted))]">Ranking-detaljer</p>
        <div className="mt-1 space-y-1 text-[11px] text-[rgb(var(--fg))]/90">
  ...
  return (
    <div className="space-y-4">
      <section className="np-node-surface rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))]/90 p-4 pt-7">
        ...
        <p className="mt-1 text-xs text-[rgb(var(--muted))]">Datagrunnlag: web-snapshot {snapshotLabel}.</p>
      </section>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {scoredCategories.map((category) => (
          <section
            key={category.id}
            className="np-node-surface min-w-0 rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))]/90 p-4 pt-7 shadow-[0_10px_30px_rgba(2,6,23,0.18)]"
          >
            ...
            <div className="mt-3 grid gap-2">
              {category.items.map((item) => (
                <article
                  key={item.id}
                  className="relative min-w-0 overflow-visible rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))]/70 px-3 py-2 transition hover:z-30"
                >
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex min-w-0 items-start gap-2">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="block min-w-0 text-sm font-semibold text-[rgb(var(--fg))] underline-offset-2 hover:underline"
                          >
                            {item.name}
                          </a>
                        </div>
                        {getHostname(item.href) ? (
                          <p className="mt-1 text-[11px] uppercase tracking-[0.08em] text-[rgb(var(--muted))]">
                            {getHostname(item.href)}
                          </p>
                        ) : null}
                      </div>
