...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const entry = await getKiOpplaringEntry("tema", slug);
  if (!entry) {
    return siteMeta({
      title: "Tema ikke funnet | KI Opplæring",
      description: "Temasiden finnes ikke eller er ikke publisert.",
      path: `/ki-opplaring/tema/${slug}`,
    });
  }
  return buildKiOpplaringEntryMetadata(entry);
}

export default async function KiOpplaringTemaDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const entry = await getKiOpplaringEntry("tema", slug);
  if (!entry) notFound();
  ...
  return <EntryDetailPage entry={entry} related={related} sectionHref="/ki-opplaring/tema" sectionTitle="Tema" />;
}
